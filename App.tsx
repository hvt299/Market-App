...
import {
  ...
  StatusBar, RefreshControl, TouchableOpacity, Modal, SectionList, Alert
} from 'react-native';
import { SafeAreaProvider, SafeAreaView } from 'react-native-safe-area-context';
import axios from 'axios';
import NetInfo from '@react-native-community/netinfo';

// --- DỮ LIỆU DANH SÁCH VÙNG 2 (Cập nhật theo QĐ 630/PLX-QĐ-TGĐ 2025) ---
const ZONE_2_DATA = [
  {
  ...
  },
  {
    ...
    // Lưu ý: Phần ghi chú trong ngoặc dựa trên cột 2 của văn bản để người dùng rõ nghĩa
    data: [
...
export default function App() {
  ...
  const [refreshing, setRefreshing] = useState(false); // Trạng thái vuốt để refresh
  const [isConnected, setIsConnected] = useState(true); // Trạng thái mạng
  const [modalVisible, setModalVisible] = useState(false); // Trạng thái hiện popup Vùng 2
  ...
  // Hàm chọn màu sắc dựa trên tên loại xăng (Trang trí)
  const getGasColor = (name: string) => {
    if (!name) return '#95A5A6';
    if (name.includes('95')) return '#e67e22'; // Màu cam cho RON 95
    if (name.includes('E5')) return '#27AE60'; // Màu xanh lá cho E5
    if (name.includes('DO') || name.includes('Dầu')) return '#2980b9'; // Màu xanh dương cho Dầu
    return '#34495e'; // Màu mặc định
  };
  ...
  // Xử lý khi người dùng vuốt xuống
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    // Lấy 2 ký tự đầu hoặc tên ngắn để làm Logo
            ...
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                <View>
                  ...
                  <Text style={styles.modalSubtitle}>Theo QĐ 630/PLX-QĐ-TGĐ (03/07/2025)</Text>
                </View>
              ...
              {/* SỬ DỤNG SECTION LIST ĐỂ HIỂN THỊ PHÂN NHÓM */}
              <SectionList
...
const styles = StyleSheet.create({
  ...
  // 5. MODAL (Cửa sổ bật lên) - Đã cập nhật cho SectionList
  modalOverlay: {
    flex: 1,
  ...
  modalContent: {
    backgroundColor: '#FFF',
    height: '85%', // Tăng chiều cao lên chút để hiển thị được nhiều hơn
  ...
  // Style cho danh sách nhóm (SectionList)
  sectionHeader: {
